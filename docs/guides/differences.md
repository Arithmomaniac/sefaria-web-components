...
| Thin client and deterministic factories have no default cache, retry, or request coalescing | A host that wants those policies must own them; repeated operations are not silently combined by the library. | The local client stays a transport capability. Sefaria's page cache and same-URL request combination are observed upstream behavior, not a local requirement. See [Current front-end cache](../evidence.md#current-front-end-cache) and [Client boundary](../design.md#client-boundary). |
...
The local client is a thin generated-client capability. It does not add a normalized domain facade, default cache, retries, or request coalescing. Component factories own projection and text processing; Web Components own layout, accessibility, interaction, and DOM rendering, but make no requests.
