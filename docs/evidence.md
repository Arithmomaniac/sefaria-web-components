...
The same pinned tool registration exposes `get_links_between_texts(reference, with_text="0")`. Its implementation calls `/api/links/{reference}` with `with_text` and returns optimized JSON as text. The planned local extension preserves that name and explicit input vocabulary while using initialized-client MCP Apps capability only when the caller omits the option.
...
The MCP tools specification reviewed on September 6, 2026 requires `structuredContent` to be a JSON object. The corrected `GET /api/links/{tref}` success response is an array, so the planned MCP connections result must wrap that unchanged response in a minimal object envelope before FastMCP serialization. This is an integration constraint and does not change the corrected generated endpoint contract.

The installed `@modelcontextprotocol/ext-apps` 1.7.5 declarations expose `getHostCapabilities().message.text` and `sendMessage` for user-role content. The installed FastMCP 3 context exposes `client_supports_extension(UI_EXTENSION_ID)` and returns false when the current client did not advertise the extension. Apps rendering capability and host text-message capability are separate negotiations, but current host implementations accept `ui/message` without advertising the optional message capability.
...
The first minimized launcher rebuilt a PowerShell command line from JSON. Windows PowerShell 5.1 deserialized the argument list as a nested object array, and `Start-Process` either rejected the array or reparsed the reconstructed string without exposing the requested CDP endpoint. The corrected Windows launcher uses Python `subprocess.Popen` with an argument vector and `SW_SHOWMINIMIZED`, preserving every VS Code switch without taking foreground focus.
