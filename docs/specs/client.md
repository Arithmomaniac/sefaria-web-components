...
- six named SDK functions generated by `@hey-api/openapi-ts` with the bundled `@hey-api/client-fetch` client
...
| `/api/shape/{title}` parameters and response | The document exposes a boolean wire value and deprecated no-op depth parameter, and models one object with inconsistent casing | The corrected request uses `"0"`/`"1"` for dependents and omits depth; the response is a list of leaf or collapsed lower-case records, plus the observed error shape |
