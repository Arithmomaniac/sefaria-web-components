...
describe("React deterministic transport", () => {
  ...
  it("rejects the wrong method, origin, path, or query", async () => {
    const fetch = createMicahFixtureFetch({ ref: "Micah 6:8" });
    const requests = [
      new Request(
        "https://example.invalid/api/v3/texts/Micah%206%3A8?version=primary&version=translation&return_format=default",
      ...
      "https://sefaria.org/api/v3/texts/Micah%206%3A8?version=primary&version=translation&return_format=default",
      "https://example.invalid/api/v3/texts/Obadiah%201%3A1?version=primary&version=translation&return_format=default",
    ...
    for (const request of requests) {
      await expect(fetch(request)).rejects.toThrow(
        "Unexpected deterministic request",
      );
    }
  });
});
